...
function getWebpackConfig(options, env) {
  ...
  if (opts.entryPoints === "<from-mappings-json>") {
    opts.entryPoints = getHtmlFilesFromMappings(opts.mappings)
  ...
  let lessLoader = { loader: require.resolve("less-loader") };
  let fileLoader = { loader: require.resolve("file-loader"), options: { ident: "file", name: "[path][name].[ext]" } };
  let rules = [
    {
      oneOf: [
  ...
  if (opts.copyFiles) {
    let htmlPostProcessedFiles = getHtmlFilesFromMappings(opts.mappings).filter(name =>
  ...
  // add copyright
  let licenceText;
  if (fs.existsSync("LICENSE")) licenceText = fs.readFileSync("LICENSE", "utf-8").trim();
  else if (fs.existsSync("../LICENSE")) licenceText = fs.readFileSync("../LICENSE", "utf-8").trim();
  else licenceText = "";
  plugins.push(
...
function getHtmlFilesFromMappings(mappingsJson) {
