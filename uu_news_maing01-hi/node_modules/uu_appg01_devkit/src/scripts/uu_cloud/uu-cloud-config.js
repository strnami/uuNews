...
class UuCloudConfig {
  ...
  uuAppBoxConfig() {
    ...
    if (cloudDescriptor.packs.length > 0) {
      result.appPackageCode = cloudDescriptor.packs[0].code;
    } else {
      result.appPackageCode = `${cloudDescriptor.code}/CMD-PACK`;
    }
  ...
  uuLogStoreConfig() {
    ...
    if (this.legacyCloudConfig) {
      ...
      if (this.legacyCloudConfig.uuLogStoreExport) {
        result.exportFrom = this.legacyCloudConfig.from;
        result.exportTo = this.legacyCloudConfig.to;
      }
    } else {
      if (!this.devkitConfig || !this.devkitConfig.appDeploymentUri) {
        throw new Error(
      ...
      result.appDeploymentUri = this.devkitConfig.appDeploymentUri;
      result.exportFrom = this.devkitConfig ? this.devkitConfig.logStoreExportFrom : null;
      result.exportTo = this.devkitConfig ? this.devkitConfig.logStoreExportTo : null;
    }
  ...
  updateUuSubAppDescriptor() {
    ...
    this.cloudDescriptorPath = this._getCloudDescriptorPath() || this._getLegacyCloudDescriptorPath(this.environment);
    let appBoxconfig = this.uuAppBoxConfig();
    let appPackUri = `${appBoxconfig.appBoxUri || appBoxconfig.appBoxUriBuilt}:${appBoxconfig.appPackageCode}`;
    let update = false;
    if (nodeSetMap) {
      Object.keys(nodeSetMap).forEach(function(nodeSetId) {
        let nodeSetCfg = nodeSetMap[nodeSetId];
