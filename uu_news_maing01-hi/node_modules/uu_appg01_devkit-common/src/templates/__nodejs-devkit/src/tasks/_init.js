...
module.exports = async function(environment, ...args) {
  ...
  let projectPackageJsonPath = Path.join(Process.cwd(), "package.json");
  ...
  let taskPrefix = selfPackageJson.name.replace(/-devkit$/, "").replace(/[\-_][\w0-9]/g, match => {
  ...
  console.log(`Project successfully initialized. You may need to run \"npm update\" to install ${selfPackageJson.name}.\n`);
};
