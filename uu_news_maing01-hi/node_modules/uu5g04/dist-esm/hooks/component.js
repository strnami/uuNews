...
function createComponent(component, isRef = false) {
  const {
    render,
    ...statics
  } = component; // If no ref is used (isRef === false) React sends bogus ref which causes warning in console => don't propagate ref

  const Comp = isRef ? forwardRef(render) : (props, ref) => render(props);

  for (let [key, value] of Object.entries(statics)) Comp[key] = value; // FIXME backward compatibility - delete after death of mixins
...
function createVisualComponent(component, isRef = false) {
  const componentCfg = { ...component,
    propTypes: { ...VISUAL_PROP_TYPES,
      ...component.propTypes
    },
    defaultProps: { ...VISUAL_DEFAULT_PROPS,
      ...component.defaultProps
    }
  };
  const Comp = isRef ? createComponentWithRef(componentCfg) : createComponent(componentCfg); // if (process.env.NODE_ENV === "development") {
