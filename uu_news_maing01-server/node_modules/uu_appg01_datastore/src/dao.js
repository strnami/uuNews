...
class UuDataStoreDao {
  constructor(name, maxNoi, maxSoi, customUri) {
    ...
    this.maxSoi = maxSoi || 16000000;
    this.maxNoi = maxNoi;
    this.defaultPageSize = 1000;
    this.noiCounts = {};
    this.customUri = customUri;
    DbConnection.init(customUri);
  ...
  async _createDefaultIndexes() {
    try {
      ...
      if (
        !e.cause ||
        !(e.cause.message == "server instance pool was destroyed" || e.cause.message == "topology was destroyed" || e.cause.message.match(/illegal state transition/))
      ) {
        throw e;
      }
    }
