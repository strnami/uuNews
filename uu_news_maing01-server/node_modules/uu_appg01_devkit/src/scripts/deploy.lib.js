...
module.exports = class Deploy {
    ...
    if (templateType === "uu5-lib" || templateType === "lib") {
      if (cdnDeploy) {
        ...
        if (token) {
          ...
          this._uploadToNexus(cdnPack, token, `${cdnName}/${version}`, cdnPackBasename);
          if (cdnDeployMajor) {
            this._uploadToNexus(cdnPack, token, `${cdnName}/${this._getMajorVersion(version)}`, cdnPackBasename);
          }
          ...
          files.forEach(({ path: file }) => {
            ...
            this._uploadToNexus(file, token, destination);
            if (cdnDeployMajor) {
  ...
  _uploadToNexus(file, token, destination, basename) {
    ...
    const request = require("request"); // TODO Use single dep, e.g. "got" and remove this one.
