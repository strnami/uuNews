...
class RoleCastService {
  ...
  constructor(uuOS8BaseUrl, castCacheTTL) {
    this._castCache = new Cache({ maxAge: castCacheTTL });
  ...
  async _verifyCastExistence(session, roleList, uuId = null) {
    ...
    if (roleList.length !== 0) {
      ...
      uuOS9RoleUris.forEach((roleUri, role) => {
        const territoryUri = roleUri.getBaseUri().toString();
        const territoryCasts = verifiedUuOS9Roles.find(r => r.territoryUri === territoryUri);
        let cacheKey = `${uuId}_${role}`;
        if (territoryCasts) {
          const rgi = territoryCasts.roleGroupIfcList.find(r => r.id === roleUri.getParameters().id || r.code === roleUri.getParameters().code);
          if (rgi) {
  ...
  async verifyUuOS9CastExistence(territoryUri, roleGroupIfcs, uuId, session) {
    ...
    let rolesSliced = [];
    let result = { territoryUri, roleGroupIfcList: [] };
    ...
    const verifiedRoles = await Promise.all(rolesSliced.map(async roles => {
      ...
      try {
        ...
        this._logger.warn(`Unable to verify cast existence in territory ${uri} for roles: ${roles}`, e);
      }
    }));
