...
const SysAppClientTokenModel = require("./models/sys-app-client-token-model");
const SysKeyStoreModel = require("./models/sys-key-store-model");
const JsonWebToken = require("jsonwebtoken");
const PemJwk = require("pem-jwk");
const Crypto = require("crypto");
const { UriBuilder } = require("uu_appg01_core-uri");
const Errors = require("./errors/sys-app-client-token-errors").AppClientService;
...
const VALIDATION_KEY_SET = "uuAppClientTokenValidation";
...
class AppClientTokenService {
  ...
  async createToken(baseUri, callUri, tokenTtl = this._tokenTtl) {
    let {kid, privateKey} = await SysAppClientTokenModel.getPrivateKey(baseUri.getAwid());
    let uuAppKey = AppKey.valueOf(
      baseUri.getVendor(),
      baseUri.getApp(),
      baseUri.getSubApp(),
      Config.get("asid"),
    ...
    if (tokenTtl > MAX_TOKEN_TTL) {
      logger.warn(`The specified ttl (${tokenTtl}) is greater than the maximum allowed value (${MAX_TOKEN_TTL}). The maximum allowed value will be used.`);
   ...
   * @param {AppKey} appKey AppKey used for validation of the token. Only non-null parts of this token are compared with the one obtained from the token.
  ...
  async isTokenValid(uri, appClientToken, clientAppBaseUri, appKey) {
    ...
    // database check
    let storedPublicKey = await SysKeyStoreModel.getKey(uri.getAwid(), VALIDATION_KEY_SET, cacheKey);
    if (storedPublicKey) {
      try {
        await promisifiedVerify(appClientToken._getTokenOrigin(), storedPublicKey.key, { clockTolerance: LEEWAY });
        this._publicKeyCache[cacheKey] = storedPublicKey.key;
        return true;
      } catch (e) {
    ...
    for (const key of keys.keys) {
      ...
      await SysKeyStoreModel.putKey(uri.getAwid(), VALIDATION_KEY_SET, cacheKey, pem);
