...
const Errors = require("./errors/app-key-errors");
...
class AppKey {
  /**
   * New instance of AppKey should be created using {@link AppKey.parse} or {@link AppKey.valueOf}.
   * @private
   */
   ...
   * @return {AppKey} instance
   */
  static parse(value) {
    if (value instanceof AppKey) {
      return value;
    }
    ...
    return new AppKey(result[1], result[2], result[3], asid, awid);
  }
  ...
  static valueOf(vendor, app, supApp, asid, awid) {
    ...
    const appKey = new AppKey(vendor, app, supApp, asid, awid);
    return appKey;
  }
   ...
   *
   * @return {string} appKey string value
   */
  toString() {
  ...
  /**
   * Returns regular expression (based on attributes of AppKey instance).
   *
   ...
   *
   * @param {AppKey} appKey AppKey with which to compare
  ...
  equals(appKey) {
    ...
    if (!appKey || !(appKey instanceof AppKey)) {
      return false;
    }
  ...
  /**
   * Compares parts of AppKey instance with set value against parts of AppKey argument.
   * This method doesn't ensure symmetric comparison (e.g. if a==b then b==a) as it compares only non-null parts of the AppKey instance it is called on.
   *
   * @param {AppKey} appKey AppKey with which to compare
